...
class BatchSampler(BatchSampler):
    """
    Custom batch sampler for dataset.
    ...
    Returns:
    Iterator: Iterator yielding batch indices.
    """

    def __init__(self, dataset, n_classes: int, n_samples: int):
...
def ssim(
    ...
    """
    Compute the Structural Similarity Index Measure (SSIM) for given inputs.

    Args:
    fmc (torch.Tensor): Input tensor.
    comb (torch.Tensor): Combined tensor.
    window_size (int): Size of the sliding window.
    sigma (float): Sigma value for SSIM calculation.
...
def Cos_sim(
    ...
    """
    Compute cosine similarity for given inputs.
